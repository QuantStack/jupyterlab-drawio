...
function activate(
  ...
  commands.addCommand('drawio:create-new', {
    ...
    execute: () => {
      ...
      commands.execute('docmanager:new-untitled', { path: cwd, type: 'file', ext: '.dio' })
        .then(model => commands.execute('docmanager:open', { path: model.path, factory: FACTORY }));
    }
  });
  ...
  commands.addCommand('drawio:export-svg', {
    ...
    isEnabled: () => (tracker.currentWidget !== null && tracker.currentWidget === app.shell.currentWidget),
    execute: () => {
      const cwd = browserFactory.defaultBrowser.model.path;
      commands.execute('docmanager:new-untitled', { path: cwd, type: 'file', ext: '.svg' })
        .then(model => {
...
function addMenus(commands: CommandRegistry, menu: IMainMenu, tracker: IDrawioTracker): void {
  // FILE MENU
  ...
  direction.addGroup([
    { command: 'drawio:command/flipH' },
    { command: 'drawio:command/flipV' }
  ], 0);
  ...
  navigation.addGroup([
    { command: 'drawio:command/collapsible' }
  ], 3);
  ...
  extrasMenu.addGroup([
    { command: 'drawio:command/editDiagram' }
  ], 1);
